-- Convert checkin_time from text to time type for precise time handling
...
begin
  ...
  -- For system operations, user can be null (service role)
  IF v_user IS NULL AND p_source != 'system_cron' THEN
    RAISE EXCEPTION 'AUTH_REQUIRED';
  END IF;

  -- For user operations, check membership
  IF v_user IS NOT NULL AND NOT public.is_member_of_challenge(p_challenge_id) THEN
  ...
  WHERE challenge_id = p_challenge_id 
    AND user_id = COALESCE(v_user, (SELECT user_id FROM public.check_ins WHERE challenge_id = p_challenge_id AND date = p_date LIMIT 1))
    AND date = p_date;
  ...
  IF v_user IS NOT NULL AND p_status = 'success' AND v_now > v_deadline THEN
    RAISE EXCEPTION 'CHECKIN_DEADLINE_PASSED';
  END IF;

  -- Determine user_id for the operation
  DECLARE
    v_target_user_id uuid := COALESCE(v_user, v_existing_row.user_id);
  BEGIN
    ...
    VALUES (p_challenge_id, v_target_user_id, p_date, p_status, p_screenshot_name, p_source, 
...
GRANT EXECUTE ON FUNCTION public.upsert_check_in_with_deadline TO authenticated;
