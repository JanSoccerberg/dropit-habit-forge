-- 1) profiles.user_name absichern: hinzufügen (falls fehlt), auffüllen, Default/Not-Null setzen
...
-- Nur setzen, wenn keine NULLs mehr vorhanden sind
do $$
begin
  if exists (select 1 from public.profiles where user_name is null) then
    -- lassen wir NOT NULL weg, bis alle gefüllt sind
    null;
  else
    alter table public.profiles
...
-- 2) ensure_profile_exists: user_name direkt setzen
...
-- 3) Leaderboard-RPCs inkl. user_name und Sicherheitslogik
